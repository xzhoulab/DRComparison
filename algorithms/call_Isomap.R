...
call_Isomap <- function(sce, num_pc, params){
	## other parameter in Isomap method
	...
	tryCatch({
		ct <- system.time({
		res_Isomap <- do.isomap(t(norm_counts), ndim = num_pc, type=c("proportion",num_prop))
		})	
		# count time
		ct <- c(user.self = ct[["user.self"]], sys.self = ct[["sys.self"]], 
            user.child = ct[["user.child"]], sys.child = ct[["sys.child"]],
            elapsed = ct[["elapsed"]])
		list(res = res_Isomap, ctimes = ct)
	},
	error = function(e) {
